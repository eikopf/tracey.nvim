...
- `lsp/tracey.lua` — Auto-discovered by Neovim 0.11+ (no setup needed). Defines base LSP config: cmd, filetypes, root_markers.
...
- `lua/tracey/commands.lua` — Dispatch table for `:Tracey` subcommands. LSP management (info/start/stop/restart/log) and CLI wrappers (status/uncovered/untested/stale/web).
- `lua/tracey/cli.lua` — Runs `tracey query <subcmd>` async via `vim.system()`, displays results in scratch buffers. Manages `tracey web` as a background job.
